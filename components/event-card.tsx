...
export function EventCard({ event }: EventCardProps) {
  ...
  const categoryColors = {
    lestur: "bg-blue-500/10 text-blue-700 dark:text-blue-400",
    námskeið: "bg-purple-500/10 text-purple-700 dark:text-purple-400",
    kynning: "bg-amber-500/10 text-amber-700 dark:text-amber-400",
    bókaklúbbur: "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400",
  }
        ...
        <div className="mt-4 pt-4 border-t space-y-3">
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <span className="text-xs">
                {new Date(event.date).toLocaleDateString("is-IS", { day: "numeric", month: "short" })}
              </span>
            </div>
            <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="text-muted-foreground">{availableSpots} sæti</span>
            </div>
          ...
          <Button className="w-full" variant="default" disabled={availableSpots === 0} asChild>
            {availableSpots === 0 ? <span>Fullbókað</span> : <Link href={`/vidburdur/${event.id}`}>Sjá nánar</Link>}
          </Button>
        </div>
      </CardContent>
    </Card>
